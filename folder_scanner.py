...
def _read_flac_tags(file_path: str) -> tuple[str, str]:
    """Return (artist, title) from a FLAC file's tags, or ('', '') on failure."""
    try:
        flac = FLAC(file_path)
        tags = flac.tags or {}
...
class FolderScannerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ── File table ────────────────────────────────────────────────── #
        tree_frame = tk.Frame(self, bg="#f5f5f5", padx=16, pady=6)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        columns = ("fpath", "ftype", "fartist", "ftitle", "fsize", "fmodified")
    ...
    def _populate_list(self, folder, show_hidden, recursive, file_count, row_index):
        ...
        for dirpath, dirnames, filenames in walker:
            ...
            for name in sorted(filenames, key=str.lower):
                ...
                artist, title = _read_flac_tags(full_path) if ext == "FLAC" else ("", "")
                ...
                self.tree.insert(
                    "", "end",
                    values=(full_path, file_type, artist, title, size, modified),
                    tags=(tag,),
                )
                file_count[0] += 1
